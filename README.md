...
X_train_l, X_test_l, y_train_l, y_test_l = train_test_split(X_lin, 
y_lin, test_size=0.2, random_state=42)
lr_model = LinearRegression() 
lr_model.fit(X_train_l, y_train_l) 
y_pred_l = lr_model.predict(X_test_l) 
print(f"Mean Squared Error: {mean_squared_error(y_test_l, 
y_pred_l):.2f}\n") 
plt.scatter(X_test_l, y_test_l, color='blue', label='Actual') 
plt.plot(X_test_l, y_pred_l, color='red', linewidth=2, 
...
df['RainTomorrow'] = 
le.fit_transform(df['RainTomorrow'].astype(str)) 
